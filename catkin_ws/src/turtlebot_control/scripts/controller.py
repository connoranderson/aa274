...
class Controller:
    ...
    def __init__(self):
        ...
        # For exact position information
        self.trans_listener = tf.TransformListener()
        rospy.Subscriber('/turtlebot_control/position_goal',
                         Float32MultiArray, self.callback_sp_pos)
        rospy.Subscriber('/turtlebot_control/velocity_goal',
                         Float32MultiArray, self.callback_sp_vel)
        rospy.Subscriber('/turtlebot_control/control_mode',
                         String, self.callback_sp_cmd)
    ...
    # Un-comment if using gazebo/model_states to update robot position
    ...
    def get_ctrl_output(self):
        ...
        # Otherwise, let getPoseControl_output determine how to get to goal
        # position.
...
# Apply saturation limits
    ...
    def getPosControl_output(self):
        ...
        rho = math.sqrt((self.x - x_g)**2 + (self.y - y_g)**2)
        alpha = wrapToPi(math.atan2(y_g - self.y, x_g - self.x) - self.th)
        delta = wrapToPi(alpha + self.th - th_g)

        # Control Law
        V = k1 * rho * math.cos(alpha)
        om = k2 * alpha + k1 * \
            (np.sinc(2 * alpha / np.pi)) * (alpha + k3 * delta)
