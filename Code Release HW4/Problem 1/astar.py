...
class AStar(object):
    ...
    def get_neighbors(self, x):
        ...
        # Initialize all possible neighbors
        solutionNeighbors = []
        for i in range(-1,2):
            for j in range(-1,2):
    ...
    def solve(self):
        ...
        while len(self.open_set)>0:
            x_cur = self.find_best_f_score() # Get the lowest cost next node

            if x_cur == self.x_goal: # If we've reached the end, this is the lowest cost solution
                # Reconstruct the path to the goal and store in self.path
            ...
            for x_neigh in self.get_neighbors(x_cur):
                if x_neigh in self.closed_set: # Check if we've already seen it before   
                    continue
                else:   
                    tentative_g_score = self.g_score[x_cur] + self.distance(x_cur,x_neigh)

                    if x_neigh not in self.open_set: # Check if neighbor is missing from this set
                    ...
                    self.f_score[x_neigh] = tentative_g_score + self.distance(x_neigh,x_goal) # Use DISTANCE function as heuristic for A* algorithm
